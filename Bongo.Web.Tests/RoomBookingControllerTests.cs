...
namespace Bongo.Web
{
    ...
    public class RoomBookingControllerTests
    {
        ...
        private RoomBookingController _bookingControler;
        [SetUp]
        public void Setup()
        {
            _studyRoomBookingService = new Mock<IStudyRoomBookingService>();
            _bookingControler = new RoomBookingController(_studyRoomBookingService.Object);
        }
        [Test]
        public void IndexPage_CallRequest_VerifyGetAllInvoked()
        {
            _bookingControler.Index();
            _studyRoomBookingService.Verify(x=> x.GetAllBooking(), Times.Once);
        }
        [Test]
        public void BookRoomCheck_ModelStateInvalid_Returnview()
        {
            _bookingControler.ModelState.AddModelError("test", "test");
            var result = _bookingControler.Book(new Models.Model.StudyRoomBooking());
        ...
        public void BookRoomCheck_NotSuccessful_NoRoomCode()
        {
            ...
            var result = _bookingControler.Book(new StudyRoomBooking());
        ...
        public void BookRoomCheck_Successful_SuccessCodeAndRedirect()
        {
            ...
            Assert.AreEqual("hello",redirectToActionResult.RouteValues["FirstName"]);
            Assert.AreEqual(StudyRoomBookingCode.Success,redirectToActionResult.RouteValues["Code"]);
        }
    }
}
